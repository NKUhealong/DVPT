...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,proj, dim, input_resolution, num_heads, window_size=7, shift_size=0,mlp_ratio=4., qkv_bias=True, drop=0., 
    ...
    def forward(self, x):
        ...
        prompt_attn = self.softmax(prompt_attn)#*self.prompt_gate  
        prompt_out = (prompt_attn @ x)
        x = x + torch.cat([prompt_out,torch.zeros(x.shape[0],x.shape[1]-self.num,x.shape[-1], dtype=x.dtype, device=x.device)], dim=1)
        #############
        
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x))) +  self.prompt_proj(x) 
        return x
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
        ...
        for i in range(depth):
            if i % 3 ==0:
                share_mlp1 = share_MLP(dim)
            blocks.append(SwinTransformerBlock(share_mlp1, dim=dim, input_resolution=input_resolution, num_heads=num_heads, 
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24),
                 ...
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,use_checkpoint=False, weight_init='', **kwargs):
        super().__init__()
...
def _create_swin_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg(SwinTransformer, variant, pretrained,default_cfg=default_cfg,img_size=img_size,
                                     num_classes=num_classes,pretrained_filter_fn=None,**kwargs)

    return model
    
def swin_base_patch4_window7_224_in22k(seg=False, **kwargs):
    img_size = 224
    model_kwargs = dict(seg = seg, img_size=img_size, in_chans=3, num_classes=5,
        patch_size=4, window_size=img_size//32, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
    return _create_swin_transformer('swin_base_patch4_window7_224_in22k', pretrained=False, **model_kwargs)