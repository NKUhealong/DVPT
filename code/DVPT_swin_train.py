...
parser.add_argument('--model-name',   type=str, default='swin_base', help='RN50x4, ViTB16')
parser.add_argument('--optimizer',    type=str, default='ADAM', help='optimizer')
parser.add_argument('--pretain',      type=int, default=True, help='log path')
parser.add_argument('--pre',          type=str, default='our_swin_Model', help='model architecture')
parser.add_argument('--input-size',   type=int, default=224, help='input size') 
parser.add_argument('--batch-size',   type=int, default=132, help='batch size')
...
def main(args):
    ...
    model =  Swin_Model(args, num_classes=5) #Swin_Model   Adapter_Swin_Model  VPT_swin_Model EVP_swin_Model SSF_swin_Model
    model = model.to(args.device)
    #print(model.state_dict().keys())
    for name,p in model.named_parameters():
        if 'fc_head'  in name: 
            p.requires_grad = True
        elif 'prompt'  in name or 'ssf'  in name:   # relative_position_bias_table norm
            p.requires_grad = True
        else:
...
def train(args, model, train_dataset, val_dataset, estimator, scaler=None):
    ...
    train_loader = DataLoader(train_dataset,batch_size=args.batch_size,shuffle=True,num_workers=8,
                              drop_last=True,pin_memory=True, sampler=None)
