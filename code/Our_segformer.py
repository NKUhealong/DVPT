...
class MLP(nn.Module):
    def __init__(self, input_dim=2048, embed_dim=768):
        ...
        self.latent_dim =10
        self.prompt_key_proj_d = nn.Linear(input_dim, self.latent_dim)
        self.prompt_key_proj_u = nn.Linear(self.latent_dim, embed_dim)
    ...
    def forward(self, x):
        x = x.flatten(2).transpose(1, 2)
        x = self.proj(x) +  self.prompt_key_proj_u(self.gellu(self.prompt_key_proj_d(x)))*self.prompt_gate 
        return x
...
class share_MLP(nn.Module):
    ...
    def forward(self, x):
        x = self.prompt_key_proj_u(self.gellu(self.prompt_key_proj_d(x)))*self.prompt_gate 
        return x

class Block(nn.Module):

    def __init__(self,proj, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.num = 10
        self.prompt = nn.Parameter(torch.zeros(1, self.num, dim))
        trunc_normal_(self.prompt, std=.02)
        self.scale = dim ** -0.5
        self.prompt_gate2 = torch.nn.Parameter(torch.zeros(1))
        self.prompt_proj = proj
        ##########
    ...
    def forward(self, x, H, W):
        ...
        x = x + torch.cat([prompt_out,torch.zeros(x.shape[0],x.shape[1]-self.num,x.shape[-1], dtype=x.dtype, device=x.device)], dim=1)*self.prompt_gate2 
        #############
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W)) +   self.prompt_proj(x) 
        return x
...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(depths[0]):
            if i % 2 ==0:
                share_mlp1 = share_MLP(embed_dims[0])
            blocks1.append(Block(share_mlp1,dim=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,sr_ratio=sr_ratios[0]))
        self.block1 = nn.ModuleList(blocks1)
        ...
        for i in range(depths[1]):
            if i % 2 ==0:
                share_mlp1 = share_MLP(embed_dims[1])
            blocks2.append(Block(share_mlp1,dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,sr_ratio=sr_ratios[1]))
        self.block2 = nn.ModuleList(blocks2)
        ...
        for i in range(depths[2]):
            if i % 2 ==0:
                share_mlp1 = share_MLP(embed_dims[2])
            blocks3.append(Block(share_mlp1,dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,sr_ratio=sr_ratios[2]))
        self.block3 = nn.ModuleList(blocks3)
        ...
        for i in range(depths[3]):
            if i % 2 ==0:
                share_mlp1 = share_MLP(embed_dims[3])
            blocks4.append(Block(share_mlp1,dim=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,sr_ratio=sr_ratios[3]))
        self.block4 = nn.ModuleList(blocks4)
...
def our_SegFormer_B4(image_size,num_classes):
